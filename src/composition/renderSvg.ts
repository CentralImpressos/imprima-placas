...
function renderGraphicElement(element: GraphicElement): string {
  switch (element.type) {
    ...
    case 'text': return `<text x="${element.x}" y="${element.y}" font-size="${element.fontSize}" font-family="${element.fontFamily ?? 'sans-serif'}" font-weight="${element.fontWeight ?? 400}" fill="${element.fill ?? '#000'}" text-anchor="${element.anchor ?? 'start'}" dominant-baseline="${element.dominantBaseline ?? 'alphabetic'}"${element.letterSpacing !== undefined ? ` letter-spacing="${element.letterSpacing}"` : ''}${element.textLength !== undefined ? ` textLength="${element.textLength}" lengthAdjust="${element.lengthAdjust ?? 'spacingAndGlyphs'}"` : ''}>${escapeXml(element.text)}</text>`;
