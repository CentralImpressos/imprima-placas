...
export interface TextElement { type: 'text'; x: number; y: number; text: string; fontFamily?: string; fontSize: number; fontWeight?: number | string; fill?: string; anchor?: 'start' | 'middle' | 'end'; dominantBaseline?: 'alphabetic' | 'middle' | 'central' | 'hanging' | 'ideographic' | 'text-before-edge' | 'text-after-edge'; letterSpacing?: number; textLength?: number; lengthAdjust?: 'spacing' | 'spacingAndGlyphs'; }
