...
import { SIGN_PRESETS, DEFAULT_FRAME_COLOR, DEFAULT_BLUE } from './data/presets';
import type { CmykColor, FrameType, PictogramPosition, SignAppearance } from './types';
...
export default function App() {
  ...
  const updatePreset=(id:string)=>setPresetId(id);
  const updateColor=(target:'frame'|'background', value:CmykColor)=>target==='frame'?setFrameColor(value):setBackgroundColor(value);
    ...
    <main className="app-layout">
      <aside className="config-panel">
        <section className="config-section"><h2>1. Modelo</h2>
          <label className="field-label">Tipo de estrutura</label><select className="field-select" value={frameType} onChange={e=>setFrameType(e.target.value as FrameType)}>{FRAME_OPTIONS.map(([v,l])=><option value={v} key={v}>{l}</option>)}</select>
          <label className="field-label">Preset</label><select className="field-select" value={presetId} onChange={e=>updatePreset(e.target.value)}>{SIGN_PRESETS.filter(p=>p.frameType===frameType).map(p=><option value={p.id} key={p.id}>{p.name}</option>)}{!SIGN_PRESETS.some(p=>p.frameType===frameType)&&<option value={presetId}>{preset.name}</option>}</select>
        </section>
        ...
        <section className="config-section"><h2>3. Cores CMYK</h2>
          <div className="color-block"><label className="field-label">Moldura / Cabeçalho</label><div className="cmyk-grid">{(['c','m','y','k'] as const).map(k=><div key={k}><span>{k.toUpperCase()}</span>{colorInput(frameColor,k,c=>updateColor('frame',c))}</div>)}</div></div>
          <div className="color-block"><label className="field-label">Fundo</label><div className="cmyk-grid">{(['c','m','y','k'] as const).map(k=><div key={k}><span>{k.toUpperCase()}</span>{colorInput(backgroundColor,k,c=>updateColor('background',c))}</div>)}</div></div>
        </section>
        <section className="config-section"><h2>4. Tamanho</h2>
          <label className="field-select-row"><span className="field-label">Dimensão</span><select className="field-select" value={custom?'custom':sizeId} onChange={e=>{if(e.target.value==='custom')setCustom(true);else{setCustom(false);setSizeId(e.target.value)}}}>{SIZES.map(s=><option value={s[0]} key={s[0]}>{s[1]} — {s[2]>s[3]?'Paisagem':s[2]===s[3]?'Quadrado':'Retrato'}</option>)}<option value="custom">Tamanho personalizado</option></select></label>
