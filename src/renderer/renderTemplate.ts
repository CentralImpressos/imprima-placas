...
function iconElements(
  svg: string, x: number, y: number, iconScale: number, ringRadiusMm: number,
  circle: boolean, prohibition: boolean, iconColor = '#000',
): GraphicElement[] {
  if (!svg) return [];
  ...
  if (prohibition) {
    elements.push({
      type: 'group', transform: `rotate(-45 ${x} ${y})`,
      children: [{ type: 'rect', x: x - ringStroke / 2, y: y - ringRadiusMm, width: ringStroke, height: ringRadiusMm * 2, fill: 'rgb(220,0,0)' }],
    });
  }
  return elements;
}
...
function fitText(message: string, preferredFontSize: number, maxWidthMm: number, maxHeightMm: number, minFontSize = 9) {
  ...
  for (let i = 0; i < 16; i += 1) {
    ...
    if ((widthOk && heightOk) || fontSize <= minFontSize) return { lines, fontSize, lineHeight };
...
export function renderTemplate(config: SignRenderConfig): string {
  ...
  const iconRgb = cmykToRgb(appearance.iconColor ?? { c: 0, m: 0, y: 0, k: 100 });
  ...
  if (frameType === 'simple' || frameType === 'header') {
    ...
    if (frameType === 'header') {
      ...
      elements.push({ type: 'text', x: cx, y: margin + headerH / 2, text: headingText, fontSize: headerFont, fontWeight: 800, fill: bg, fontFamily: 'Barlow Semi Condensed, sans-serif', anchor: 'middle', dominantBaseline: 'middle' });
    }
  ...
  if (hasIcon && position === 'top') {
    ...
    const textTargetW = wordCount <= 2 ? Math.min(usableWidth, pictSize) : Math.min(usableWidth, Math.max(pictSize, usableWidth * 0.92));
    ...
    if (blockH > usableHeight && pictSize > minPict) {
      ...
      textMaxH = Math.max(18, usableHeight - pictSize - gap);
      const tw = wordCount <= 2 ? Math.min(usableWidth, pictSize) : Math.min(usableWidth, Math.max(pictSize, usableWidth * 0.92));
  ...
  finalLines.forEach((line, index) => elements.push({
    type: 'text', x: textX, y: textY + (index - (finalLines.length - 1) / 2) * finalLineHeight,
    text: line, fontSize: finalFontSize, fontWeight: 800, fill: '#000',
    fontFamily: 'Barlow Semi Condensed, sans-serif', anchor: textAnchor, dominantBaseline: 'middle',
  }));
